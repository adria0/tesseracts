use web3::types::{Address,H256,U256,Block,BlockId,TransactionId,Transaction,TransactionReceipt,BlockNumber,Bytes};
...
impl<'a> BlockchainReader<'a> {
    ...
    pub fn block_with_txs(&self, blockno : u64) -> Result<Option<Block<Transaction>>,Error> {
        ...
        if let Some(blk) = self.db.get_block(blockno)? {
            ...
            Ok(Some(into_block(blk, move |h:H256| txs.remove(&h).unwrap())))
        } else {
            let blockid = BlockId::Number(BlockNumber::Number(blockno));
    ...
    pub fn tx(&self, txhash: H256) -> Result<Option<(Transaction,Option<TransactionReceipt>)>,Error> {
        let mut tx = self.db.get_tx(&txhash)?;
        if tx.is_none() {
            tx = self.wc.web3.eth().transaction(TransactionId::Hash(txhash)).wait()?;
        }
        if let Some(tx) = tx {
